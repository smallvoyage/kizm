...
describe("getWeeklyReview", () => {
  test("食事の記録が不足すると比較せず、身体組成の前週差は維持する", () => {
    const logs = [
    ...
    const review = getWeeklyReview(logs, "2026-08-10", "2026-08-17")

    expect(review?.nutrition.averages).toMatchObject({
      calories: { value: 2_300, previousDifference: null, recordedDays: 2 },
      protein: { value: 130, previousDifference: null, recordedDays: 2 },
      fat: { value: 65, previousDifference: null, recordedDays: 2 },
      carbs: { value: 260, previousDifference: null, recordedDays: 2 },
    })
  ...
  test("記録がない週は平均、前週差、身体組成の変化をnullで返す", () => {
    const review = getWeeklyReview([], "2026-08-10", "2026-08-17")
  ...
  test("身体組成はnullを飛ばした開始値、終了値、差を返す", () => {
    ...
    const bodyComposition = getWeeklyReview(
      logs,
      "2026-08-10",
      "2026-08-17"
    )?.bodyComposition
